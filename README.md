...
> A local-first personal workbench: todos, notes, pomodoro, habit tracking, activity heatmap, an AI assistant and a weekly report. All data stays in local JSON files.
...
- **总览** — 两行三列卡片看板,拖拽互换位置并记忆布局;侧边导航可拖拽排序、可收起;右下角 AI 悬浮球随处唤起就地聊天
...
- **AI 助手** — 对话式工作台助手:AI 能读取你的待办/笔记/习惯/番茄数据做问答与复盘,也能直接帮你建待办、勾待办、删待办、写笔记、建习惯、打卡、切换页面(见下方「AI 助手配置」)
...
- 数据存放在 `data/*.json`(`todos` / `notes` / `habits` / `pomodoros` / `chats` / `links`),带缩进,可直接查看
